...
async def handle_business_message(message: Message, bot: Bot, config: Config) -> None:
    ...
    # If view-once media — immediately forward to owner
    if data["is_view_once"] and data["file_id"]:
        await _forward_view_once(bot, config, data)


async def _forward_view_once(bot: Bot, config: Config, data: dict) -> None:
    """Send the view-once media to the bot owner."""
    ...
    try:
        ...
        logger.info("Forwarded view-once %s to owner", media_type)
